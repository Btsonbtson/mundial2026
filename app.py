...
if 'cloud_url' not in st.session_state: st.session_state.cloud_url = "https://file.io/dummy" # Αρχικό dummy
...
# 🔄 ΔΙΑΒΑΣΜΑ EXCEL (Αν υπάρχει ήδη στη μνήμη του session, αλλιώς φτιάχνει ένα προσωρινό)
try:
    wb_read = openpyxl.load_workbook("Mundial 2026.xlsx", data_only=True)
except:
    # Αν για κάποιο λόγο δεν το βρει, φτιάχνει dummy για να μην κρασάρει η οθόνη
...
# Κουμπί Έξοδος & LIVE Αποθήκευση
if st.button("🚪 ΕΞΟΔΟΣ & ΑΥΤΟΜΑΤΗ ΑΠΟΘΗΚΕΥΣΗ"):
    with st.spinner("Γίνεται αποθήκευση..."):
        try:
            wb_save = openpyxl.load_workbook("Mundial 2026.xlsx")
        except:
        ...
        wb_save.save("Mundial 2026.xlsx")
        
        st.session_state.temp_matches = {}
        st.session_state.temp_groups = {}
        st.session_state.logged_in_user = None
        st.success("Οι αλλαγές αποθηκεύτηκαν τοπικά στον Cloud Server!")
        st.rerun()
...
with tab2:
    ...
    group_teams = []
    for r in range(4, 8):
        try: val = sheet_read[f"{col_letter}{r}"].value
        except: val = None
    ...
    if len(group_teams) == 4:
        if col_letter not in st.session_state.temp_groups:
            try:
                g0 = str(sheet_read[f"{col_letter}{start_row}"].value).strip() if sheet_read[f"{col_letter}{start_row}"].value else group_teams[0]
                g1 = str(sheet_read[f"{col_letter}{start_row+1}"].value).strip() if sheet_read[f"{col_letter}{start_row+1}"].value else group_teams[1]
                g2 = str(sheet_read[f"{col_letter}{start_row+2}"].value).strip() if sheet_read[f"{col_letter}{start_row+2}"].value else group_teams[2]
                g3 = str(sheet_read[f"{col_letter}{start_row+3}"].value).strip() if sheet_read[f"{col_letter}{start_row+3}"].value else group_teams[3]
            except:
                g0, g1, g2, g3 = group_teams[0], group_teams[1], group_teams[2], group_teams[3]
            st.session_state.temp_groups[col_letter] = [g0, g1, g2, g3]
...
with tab3:
    ...
    st.write("Κατεβάστε την τελευταία έκδοση του αρχείου με όλες τις φόρμουλες και τις βαθμολογίες σας!")
    try:
        with open("Mundial 2026.xlsx", "rb") as f:
            excel_bytes = f.read()
        st.download_button(
        ...
        st.error("Το αρχείο δεν είναι έτοιμο ακόμα.")
